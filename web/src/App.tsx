...
function statusClass(status: CampaignStatus) {
  if (status === "running") return "bg-amber-500/20 text-amber-200";
  if (status === "completed") return "bg-emerald-500/20 text-emerald-200";
  if (status === "stopped") return "bg-rose-500/20 text-rose-200";
  return "bg-zinc-800 text-zinc-200";
}
function noticeClass(tone: NoticeTone) {
  if (tone === "success") return "border-emerald-500/40 bg-emerald-500/20 text-emerald-200";
  if (tone === "error") return "border-rose-500/40 bg-rose-500/20 text-rose-200";
...
function connectorStatusClass(status: ConnectorHealthStatus) {
  ...
  if (status === "unconfigured") return "bg-zinc-800 text-zinc-200";
  return "bg-rose-500/20 text-rose-200";
}
function groupStatusClass(status: GroupPostStatus) {
  if (status === "queued") return "bg-zinc-800 text-zinc-200";
  if (status === "processing") return "bg-sky-500/20 text-sky-200";
  if (status === "sent") return "bg-emerald-500/20 text-emerald-200";
  return "bg-rose-500/20 text-rose-200";
}
...
export function App() {
  ...
  return (
    <div className="dark min-h-screen app-background px-4 py-5 text-zinc-100 lg:px-8">
      <div className="mx-auto max-w-[1440px]">
        <header className="surface-panel fade-rise flex flex-wrap items-center justify-between gap-3 rounded-3xl p-5">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-400">Ops-first control plane for sessions, queue runtime, and broadcast execution.</p>
          </div>
          <div className="flex items-center gap-2">
            <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium ${health === "healthy" ? "bg-emerald-500/20 text-emerald-200" : health === "down" ? "bg-rose-500/20 text-rose-200" : "bg-zinc-800 text-zinc-300"}`}>
              {health === "healthy" ? <CheckCircle2 className="mr-1 h-3.5 w-3.5" /> : <AlertCircle className="mr-1 h-3.5 w-3.5" />}
              API {health}
            </span>
            <button type="button" onClick={() => void refreshAll()} disabled={Boolean(busy.refresh)} className="inline-flex items-center rounded-xl border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-xs hover:bg-zinc-800 disabled:opacity-60">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[270px_minmax(0,1fr)]">
          <aside className="surface-panel rounded-3xl p-3 lg:sticky lg:top-5 lg:h-[calc(100vh-72px)] lg:overflow-auto">
            <div className="space-y-1">
              {nav.map((item) => {
                ...
                return (
                  <button key={item.id} type="button" onClick={() => { setScreen(item.id); if (item.id === "ops") setOpsView("dashboard"); if (item.id === "broadcast") setBroadcastView("campaign_create"); }} className={`w-full rounded-2xl px-3 py-2 text-left ${active ? "bg-emerald-600 text-white" : "hover:bg-zinc-800"}`}>
                    <div className="flex items-center gap-2 text-sm font-medium"><Icon className="h-4 w-4" />{item.label}</div>
                  </button>
                );
              })}
            </div>
            <div className="mt-4 rounded-2xl bg-zinc-900/70 p-3 text-sm">
              <p className="text-xs uppercase tracking-wide text-zinc-400">Live Snapshot</p>
              <div className="mt-2 space-y-1">
                <div className="flex justify-between"><span>Agent Pending</span><span className="font-semibold text-indigo-300">{pendingActionCount}</span></div>
                <div className="flex justify-between"><span>Queue Ready</span><span className={`font-semibold ${queueRuntime?.ready ? "text-emerald-300" : "text-rose-300"}`}>{queueRuntime ? String(queueRuntime.ready) : "-"}</span></div>
                <div className="flex justify-between"><span>Group Queued</span><span className="font-semibold">{groupStatus?.queue.queued ?? "-"}</span></div>
                <div className="flex justify-between"><span>Campaigns</span><span className="font-semibold text-amber-300">{campaignStats.total}</span></div>
              </div>
            </div>
          </aside>
          ...
          <main className="space-y-4">
            {screen === "ops" && (
              <section className="surface-panel rounded-3xl p-3">
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={() => setOpsView("dashboard")} className={`rounded-xl px-3 py-1.5 text-sm ${opsView === "dashboard" ? "bg-emerald-600 text-white" : "border border-zinc-700 bg-zinc-900 hover:bg-zinc-800"}`}>Dashboard</button>
                  <button type="button" onClick={() => setOpsView("consents")} className={`rounded-xl px-3 py-1.5 text-sm ${opsView === "consents" ? "bg-emerald-600 text-white" : "border border-zinc-700 bg-zinc-900 hover:bg-zinc-800"}`}>Consents</button>
                  <button type="button" onClick={() => setOpsView("intent_lab")} className={`rounded-xl px-3 py-1.5 text-sm ${opsView === "intent_lab" ? "bg-emerald-600 text-white" : "border border-zinc-700 bg-zinc-900 hover:bg-zinc-800"}`}>Intent</button>
                  <button type="button" onClick={() => setOpsView("audit")} className={`rounded-xl px-3 py-1.5 text-sm ${opsView === "audit" ? "bg-emerald-600 text-white" : "border border-zinc-700 bg-zinc-900 hover:bg-zinc-800"}`}>Audit</button>
                </div>
              </section>
            )}
            ...
            {screen === "broadcast" && (
              <section className="surface-panel rounded-3xl p-3">
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={() => setBroadcastView("campaign_create")} className={`rounded-xl px-3 py-1.5 text-sm ${broadcastView === "campaign_create" ? "bg-emerald-600 text-white" : "border border-zinc-700 bg-zinc-900 hover:bg-zinc-800"}`}>Campaign Studio</button>
                  <button type="button" onClick={() => setBroadcastView("campaign_ops")} className={`rounded-xl px-3 py-1.5 text-sm ${broadcastView === "campaign_ops" ? "bg-emerald-600 text-white" : "border border-zinc-700 bg-zinc-900 hover:bg-zinc-800"}`}>Campaign Ops</button>
                </div>
              </section>
            )}
            ...
            {screen === "ops" && opsView === "dashboard" && (
              <section className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  <div className="surface-panel rounded-3xl p-4"><p className="text-xs uppercase text-zinc-400">Consents</p><p className="mt-1 text-2xl font-semibold">{consentStats.total}</p></div>
                  <div className="surface-panel rounded-3xl p-4"><p className="text-xs uppercase text-zinc-400">Drafts</p><p className="mt-1 text-2xl font-semibold">{campaignStats.draft}</p></div>
                  <div className="surface-panel rounded-3xl p-4"><p className="text-xs uppercase text-zinc-400">Running</p><p className="mt-1 text-2xl font-semibold">{campaignStats.run}</p></div>
                  <div className="surface-panel rounded-3xl p-4"><p className="text-xs uppercase text-zinc-400">Role</p><p className="mt-1 text-lg font-semibold">{config.role}</p><p className="mt-2 text-xs text-zinc-400">Connectors: {connectorStats.healthy}/{connectorStats.total}</p></div>
                </div>
                <div className="surface-panel rounded-3xl p-4">
                  <div className="mb-2 flex items-center justify-between"><h2 className="text-base font-semibold">Connector Health</h2><button type="button" onClick={() => void loadConnectorHealth()} className="rounded-xl border border-zinc-700 px-3 py-1.5 text-xs">Refresh</button></div>
                  <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="text-left text-xs uppercase text-zinc-400"><th className="pb-2 pr-3">Connector</th><th className="pb-2 pr-3">Status</th><th className="pb-2 pr-3">Provider</th><th className="pb-2 pr-3">Checks</th></tr></thead><tbody className="divide-y divide-zinc-800">{(connectorSnapshot?.connectors || []).map((item) => <tr key={item.connector.id}><td className="py-2 pr-3"><div className="font-medium">{item.connector.name}</div><div className="text-xs text-zinc-400">{item.connector.domain}</div></td><td className="py-2 pr-3"><span className={`rounded-full px-2 py-0.5 text-xs ${connectorStatusClass(item.status)}`}>{item.status}</span></td><td className="py-2 pr-3">{item.connector.provider}</td><td className="py-2 pr-3">{item.checks.map((check) => check.name).join(", ") || "-"}</td></tr>)}{(!connectorSnapshot || connectorSnapshot.connectors.length === 0) && <tr><td colSpan={4} className="py-6 text-center text-zinc-400">No connector snapshot.</td></tr>}</tbody></table></div>
                </div>
                <div className="surface-panel rounded-3xl p-4">
                  <h2 className="mb-3 text-base font-semibold">Latest Campaigns</h2>
                  <div className="overflow-x-auto">
                    <table className="min-w-full text-sm">
                      <thead><tr className="text-left text-xs uppercase text-zinc-400"><th className="pb-2 pr-3">Name</th><th className="pb-2 pr-3">Status</th><th className="pb-2 pr-3">Audience</th><th className="pb-2 pr-3">Updated</th></tr></thead>
                      <tbody className="divide-y divide-zinc-800">
                        {campaigns.slice(0, 8).map((c) => <tr key={c.id}><td className="py-2 pr-3"><div className="font-medium">{c.name}</div><div className="text-xs text-zinc-400">{c.id}</div></td><td className="py-2 pr-3"><span className={`rounded-full px-2 py-0.5 text-xs ${statusClass(c.status)}`}>{c.status}</span></td><td className="py-2 pr-3">{c.audience.length}</td><td className="py-2 pr-3 text-zinc-400">{fmt(c.lastRunAtIso || c.createdAtIso)}</td></tr>)}
                        {campaigns.length === 0 && <tr><td colSpan={4} className="py-6 text-center text-zinc-400">No campaigns yet.</td></tr>}
                      </tbody>
                    </table>
                  </div>
                </div>
            ...
            {screen === "ops" && opsView === "consents" && (
              <section className="space-y-4">
                <div className="grid gap-4 xl:grid-cols-3">
                  <div className="surface-panel rounded-3xl p-4"><h2 className="text-base font-semibold">Add Consent</h2><input value={addPhone} onChange={(e) => setAddPhone(e.target.value)} placeholder="+919876543210" className="mt-2 w-full rounded-xl border border-zinc-700 px-3 py-2 text-sm" /><button type="button" onClick={() => void addConsent()} disabled={Boolean(busy.add)} className="mt-2 inline-flex items-center rounded-xl bg-emerald-600 px-3 py-2 text-sm text-white disabled:opacity-60">{busy.add ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <ShieldCheck className="mr-1 h-4 w-4" />}Add</button></div>
                  <div className="surface-panel rounded-3xl p-4"><h2 className="text-base font-semibold">Revoke Consent</h2><input value={revokePhone} onChange={(e) => setRevokePhone(e.target.value)} placeholder="+919876543210" className="mt-2 w-full rounded-xl border border-zinc-700 px-3 py-2 text-sm" /><button type="button" onClick={() => void revokeConsent()} disabled={Boolean(busy.revoke)} className="mt-2 inline-flex items-center rounded-xl bg-rose-600 px-3 py-2 text-sm text-white disabled:opacity-60">{busy.revoke ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <AlertCircle className="mr-1 h-4 w-4" />}Revoke</button></div>
                  <div className="surface-panel rounded-3xl p-4"><h2 className="text-base font-semibold">Lookup</h2><input value={lookupPhone} onChange={(e) => setLookupPhone(e.target.value)} placeholder="+919876543210" className="mt-2 w-full rounded-xl border border-zinc-700 px-3 py-2 text-sm" /><button type="button" onClick={() => void lookupConsent()} disabled={Boolean(busy.lookup)} className="mt-2 inline-flex items-center rounded-xl bg-sky-600 px-3 py-2 text-sm text-white disabled:opacity-60">Check</button>{lookupStatus && <div className="mt-2 rounded-xl bg-zinc-900/70 p-2 text-xs">{lookupStatus}</div>}</div>
                </div>
                <div className="surface-panel rounded-3xl p-4">
                  <div className="mb-3 flex items-center justify-between"><h2 className="text-base font-semibold">Consent Records</h2><div className="flex gap-2"><select value={consentFilter} onChange={(e) => setConsentFilter(e.target.value as "all" | ConsentStatus)} className="rounded-xl border border-zinc-700 px-2 py-1.5 text-sm"><option value="all">all</option><option value="opted_in">opted_in</option><option value="opted_out">opted_out</option></select><button type="button" onClick={() => void loadConsents()} className="rounded-xl border border-zinc-700 px-3 py-1.5 text-sm">Refresh</button></div></div>
                  <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="text-left text-xs uppercase text-zinc-400"><th className="pb-2 pr-3">Phone</th><th className="pb-2 pr-3">Status</th><th className="pb-2 pr-3">Purpose</th><th className="pb-2 pr-3">Updated</th></tr></thead><tbody className="divide-y divide-zinc-800">{consents.map((c) => <tr key={c.phoneE164}><td className="py-2 pr-3 font-medium">{c.phoneE164}</td><td className="py-2 pr-3"><span className={`rounded-full px-2 py-0.5 text-xs ${c.status === "opted_in" ? "bg-emerald-500/20 text-emerald-200" : "bg-rose-500/20 text-rose-200"}`}>{c.status}</span></td><td className="py-2 pr-3">{c.purpose}</td><td className="py-2 pr-3 text-zinc-400">{fmt(c.updatedAtIso)}</td></tr>)}{consents.length === 0 && <tr><td colSpan={4} className="py-6 text-center text-zinc-400">No records.</td></tr>}</tbody></table></div>
                </div>
              </section>
            )}
              ...
              <section className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
                <div className="surface-panel rounded-3xl p-4">
                  ...
                  <div className="mt-3 grid gap-2 md:grid-cols-2">
                    <input value={campaignForm.name} onChange={(e) => setCampaignForm((p) => ({ ...p, name: e.target.value }))} placeholder="name" className="rounded-xl border border-zinc-700 px-3 py-2 text-sm" />
                    <input value={campaignForm.client} onChange={(e) => setCampaignForm((p) => ({ ...p, client: e.target.value }))} placeholder="client" className="rounded-xl border border-zinc-700 px-3 py-2 text-sm" />
                    <input value={campaignForm.templateName} onChange={(e) => setCampaignForm((p) => ({ ...p, templateName: e.target.value }))} placeholder="templateName" className="rounded-xl border border-zinc-700 px-3 py-2 text-sm" />
                    <input value={campaignForm.language} onChange={(e) => setCampaignForm((p) => ({ ...p, language: e.target.value }))} placeholder="en" className="rounded-xl border border-zinc-700 px-3 py-2 text-sm" />
                    <select value={campaignForm.category} onChange={(e) => { const category = e.target.value as CampaignCategory; setCampaignForm((p) => ({ ...p, category, consentMode: category === "marketing" ? "required" : "optional", requireApproval: category === "marketing" })); }} className="rounded-xl border border-zinc-700 px-3 py-2 text-sm"><option value="marketing">marketing</option><option value="utility">utility</option></select>
                    <select value={campaignForm.consentMode} onChange={(e) => setCampaignForm((p) => ({ ...p, consentMode: e.target.value as ConsentMode }))} className="rounded-xl border border-zinc-700 px-3 py-2 text-sm"><option value="required">required</option><option value="optional">optional</option><option value="disabled">disabled</option></select>
                  </div>
                  <input value={campaignForm.reraProjectId} onChange={(e) => setCampaignForm((p) => ({ ...p, reraProjectId: e.target.value }))} placeholder="reraProjectId" className="mt-2 w-full rounded-xl border border-zinc-700 px-3 py-2 text-sm" />
                  <div className="mt-2 rounded-2xl border border-zinc-700 bg-zinc-900/70 p-3">
                    <div className="mb-2 flex items-center justify-between"><p className="text-sm font-semibold">Bulk Number Upload</p><select value={bulkUploadMode} onChange={(e) => setBulkUploadMode(e.target.value as BulkUploadMode)} className="rounded-xl border border-zinc-700 px-2 py-1 text-xs"><option value="append">append</option><option value="replace">replace</option></select></div>
                    <p className="text-xs text-zinc-400">Upload CSV/TXT with numbers. Detected numbers are normalized to E.164 before adding to audience.</p>
                    ...
                    <textarea value={bulkPasteRaw} onChange={(e) => setBulkPasteRaw(e.target.value)} rows={3} placeholder="Or paste numbers/CSV rows here" className="mt-2 w-full rounded-xl border border-zinc-700 px-3 py-2 text-sm" />
                    <div className="mt-2 flex items-center gap-2"><button type="button" onClick={() => { applyBulkNumbers(bulkPasteRaw, "pasted text"); setBulkPasteRaw(""); }} className="rounded-xl border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-xs">Apply Pasted</button>{bulkUploadSummary && <span className="text-xs text-zinc-400">{bulkUploadSummary}</span>}</div>
                  </div>
                  <textarea value={campaignForm.audienceRaw} onChange={(e) => setCampaignForm((p) => ({ ...p, audienceRaw: e.target.value }))} rows={6} placeholder="+919..., +918..." className="mt-2 w-full rounded-xl border border-zinc-700 px-3 py-2 text-sm" />
            ...
            {screen === "broadcast" && broadcastView === "campaign_ops" && (
              <section className="grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
                <div className="surface-panel rounded-3xl p-4"><div className="mb-2 flex items-center justify-between"><h2 className="text-base font-semibold">Campaigns</h2><button type="button" onClick={() => void loadCampaigns()} className="rounded-xl border border-zinc-700 px-2 py-1 text-xs">Refresh</button></div><div className="space-y-2">{campaigns.map((c) => <button key={c.id} type="button" onClick={() => setSelectedCampaignId(c.id)} className={`w-full rounded-xl border px-3 py-2 text-left ${selectedCampaignId === c.id ? "border-emerald-500/60 bg-emerald-500/20" : "border-zinc-700 bg-zinc-900 hover:bg-zinc-800"}`}><div className="flex items-center justify-between"><span className="truncate text-sm font-medium">{c.name}</span><span className={`rounded-full px-2 py-0.5 text-[11px] ${statusClass(c.status)}`}>{c.status}</span></div><p className="text-xs text-zinc-400">{c.id}</p></button>)}{campaigns.length === 0 && <p className="text-sm text-zinc-400">No campaigns.</p>}</div></div>
                <div className="space-y-4">
                  <div className="surface-panel rounded-3xl p-4"><h2 className="text-base font-semibold">Selected Campaign</h2>{selectedCampaign ? <div className="mt-3 space-y-2 text-sm"><div className="rounded-xl bg-zinc-900/70 p-3"><p className="font-medium">{selectedCampaign.name}</p><p className="text-xs text-zinc-400">{selectedCampaign.id}</p><p className="text-xs">audience={selectedCampaign.audience.length}</p><p className="text-xs">consentMode={selectedCampaign.compliance.consentMode}</p></div><div><div className="mb-1 flex justify-between text-xs"><span>Run Progress</span><span>{progress}%</span></div><div className="h-2 rounded-full bg-zinc-800"><div className="h-2 rounded-full bg-emerald-500" style={{ width: `${Math.max(0, Math.min(100, progress))}%` }} /></div></div>{selectedCampaign.lastPolicyCheck && <div className="rounded-xl border border-zinc-700 p-2 text-xs">preflight={selectedCampaign.lastPolicyCheck.ok ? "ok" : "blocked"} at {fmt(selectedCampaign.lastPolicyCheck.atIso)}</div>}</div> : <p className="mt-2 text-sm text-zinc-400">Select a campaign.</p>}</div>
                  <div className="surface-panel rounded-3xl p-4"><h3 className="text-base font-semibold">Controls</h3><div className="mt-2 flex flex-wrap items-center gap-2"><button type="button" onClick={() => void preflightCampaign()} disabled={!selectedCampaignId || Boolean(busy.preflight)} className="inline-flex items-center rounded-xl bg-sky-600 px-3 py-2 text-sm text-white disabled:opacity-60"><FileCheck2 className="mr-1 h-4 w-4" />Preflight</button><button type="button" onClick={() => void approveCampaign()} disabled={!selectedCampaignId || Boolean(busy.approve)} className="inline-flex items-center rounded-xl bg-indigo-600 px-3 py-2 text-sm text-white disabled:opacity-60"><ShieldCheck className="mr-1 h-4 w-4" />Approve</button><label className="inline-flex items-center gap-1 rounded-xl border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs"><input type="checkbox" checked={dryRun} onChange={(e) => setDryRun(e.target.checked)} />dryRun</label><button type="button" onClick={() => void runCampaign()} disabled={!selectedCampaignId || Boolean(busy.run)} className="inline-flex items-center rounded-xl bg-emerald-600 px-3 py-2 text-sm text-white disabled:opacity-60">{busy.run ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <PlayCircle className="mr-1 h-4 w-4" />}Run</button></div><div className="mt-2 grid gap-2 md:grid-cols-2"><input value={approvedBy} onChange={(e) => setApprovedBy(e.target.value)} placeholder="approvedBy" className="rounded-xl border border-zinc-700 px-3 py-2 text-sm" /><input value={approvalNote} onChange={(e) => setApprovalNote(e.target.value)} placeholder="note" className="rounded-xl border border-zinc-700 px-3 py-2 text-sm" /></div></div>
                  <div className="surface-panel rounded-3xl p-4"><h3 className="text-base font-semibold">Run Recipients</h3><div className="mt-2 overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="text-left text-xs uppercase text-zinc-400"><th className="pb-2 pr-3">phone</th><th className="pb-2 pr-3">action</th><th className="pb-2 pr-3">reason</th></tr></thead><tbody className="divide-y divide-zinc-800">{runRecipients.map((r) => <tr key={`${r.phone}-${r.action}-${r.reason || ""}`}><td className="py-2 pr-3">{r.phone}</td><td className="py-2 pr-3">{r.action}</td><td className="py-2 pr-3 text-zinc-400">{r.reason || "-"}</td></tr>)}{runRecipients.length === 0 && <tr><td colSpan={3} className="py-5 text-center text-zinc-400">No run output yet.</td></tr>}</tbody></table></div></div>
                </div>
              </section>
            )}
            ...
            {screen === "ops" && opsView === "intent_lab" && (
              <section className="grid gap-4 xl:grid-cols-[1fr_0.9fr]">
                <div className="surface-panel rounded-3xl p-4"><h2 className="text-base font-semibold">Intent Lab</h2><textarea value={intentText} onChange={(e) => setIntentText(e.target.value)} rows={7} placeholder="Send me 2BHK pricing in Whitefield" className="mt-2 w-full rounded-xl border border-zinc-700 px-3 py-2 text-sm" /><div className="mt-2 grid gap-2 md:grid-cols-[auto_1fr]"><label className="inline-flex items-center gap-2 rounded-xl border border-zinc-700 px-3 py-2 text-sm"><input type="checkbox" checked={intentUseAi} onChange={(e) => setIntentUseAi(e.target.checked)} />useAi</label><input value={intentModel} onChange={(e) => setIntentModel(e.target.value)} placeholder="model override" className="rounded-xl border border-zinc-700 px-3 py-2 text-sm" /></div><button type="button" onClick={() => void classifyIntent()} disabled={Boolean(busy.intent)} className="mt-2 inline-flex items-center rounded-xl bg-emerald-600 px-3 py-2 text-sm text-white disabled:opacity-60">{busy.intent ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <FlaskConical className="mr-1 h-4 w-4" />}Classify</button></div>
                <div className="surface-panel rounded-3xl p-4"><h3 className="text-base font-semibold">Output</h3>{intentResult ? <div className="mt-2 space-y-2 text-sm"><div className="rounded-xl bg-zinc-900/70 p-3"><p>intent: <span className="font-semibold">{intentResult.intent}</span></p><p>confidence: <span className="font-semibold">{Math.round(intentResult.confidence * 100)}%</span></p><p>route: <span className="font-semibold">{intentResult.route}</span></p><p>provider: <span className="font-semibold">{intentResult.provider}</span></p></div><pre className="max-h-64 overflow-auto rounded-xl bg-zinc-950 p-3 text-xs text-emerald-200">{JSON.stringify(intentResult.fields, null, 2)}</pre></div> : <p className="mt-2 text-sm text-zinc-400">Run classification to inspect fields.</p>}</div>
              </section>
            )}

            {screen === "ops" && opsView === "audit" && (
              <section className="surface-panel rounded-3xl p-4"><div className="mb-2 flex items-center justify-between"><h2 className="text-base font-semibold">Audit Timeline</h2><button type="button" onClick={clearAudit} className="rounded-xl border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-xs">Clear</button></div><div className="space-y-2">{audit.map((a) => <div key={a.id} className="rounded-xl border border-zinc-700 bg-zinc-900 p-3"><div className="flex items-center justify-between"><p className="text-sm font-medium">{a.action}</p><p className="text-xs text-zinc-400">{fmt(a.atIso)}</p></div><p className="mt-1 text-sm text-zinc-300">{a.details}</p></div>)}{audit.length === 0 && <p className="text-sm text-zinc-400">No events yet.</p>}</div></section>
            )}

            {screen === "agent" && (
              <section className="grid gap-4 xl:grid-cols-[340px_minmax(0,1fr)]">
                <div className="surface-panel rounded-3xl p-4"><div className="mb-2 flex items-center justify-between"><h2 className="text-base font-semibold">Sessions</h2><div className="flex gap-2"><button type="button" onClick={() => void startAgentSession()} disabled={Boolean(busy.agent_start)} className="rounded-xl bg-emerald-600 px-3 py-1.5 text-xs text-white disabled:opacity-60">{busy.agent_start ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Start"}</button><button type="button" onClick={() => void loadAgentSessions()} className="rounded-xl border border-zinc-700 px-3 py-1.5 text-xs">Refresh</button></div></div><div className="space-y-2">{agentSessions.map((session) => <button key={session.id} type="button" onClick={() => setSelectedSessionId(session.id)} className={`w-full rounded-xl border px-3 py-2 text-left ${selectedSessionId === session.id ? "border-emerald-500/60 bg-emerald-500/20" : "border-zinc-700 bg-zinc-900 hover:bg-zinc-800"}`}><div className="flex items-center justify-between"><span className="truncate text-sm font-medium">{session.id}</span><span className="rounded-full bg-zinc-800 px-2 py-0.5 text-[11px]">{session.pendingActions.length}</span></div><p className="text-xs text-zinc-400">{fmt(session.updatedAtIso)}</p></button>)}{agentSessions.length === 0 && <p className="text-sm text-zinc-400">No sessions yet.</p>}</div></div>
                <div className="space-y-4">
                  <div className="surface-panel rounded-3xl p-4"><h2 className="text-base font-semibold">Session Actions</h2>{selectedSession ? <div className="mt-2 space-y-2"><div className="rounded-xl bg-zinc-900/70 p-3 text-sm"><p className="font-medium">{selectedSession.id}</p><p className="text-xs text-zinc-400">turns={selectedSession.turns} pending={selectedSession.pendingActions.length}</p></div><div className="grid gap-2 md:grid-cols-[140px_minmax(0,1fr)_auto]"><select value={agentAutonomy} onChange={(e) => setAgentAutonomy(Number(e.target.value) as 0 | 1 | 2)} className="rounded-xl border border-zinc-700 px-2 py-2 text-sm"><option value={0}>autonomy 0</option><option value={1}>autonomy 1</option><option value={2}>autonomy 2</option></select><input value={agentMessage} onChange={(e) => setAgentMessage(e.target.value)} placeholder="Send agent message..." className="rounded-xl border border-zinc-700 px-3 py-2 text-sm" /><button type="button" onClick={() => void sendAgentMessage()} disabled={Boolean(busy.agent_message)} className="rounded-xl bg-emerald-600 px-3 py-2 text-sm text-white disabled:opacity-60">{busy.agent_message ? <Loader2 className="h-4 w-4 animate-spin" /> : "Send"}</button></div><div className="flex gap-2"><button type="button" onClick={() => void approveAgentAction()} disabled={!selectedSession.pendingActions.length || Boolean(busy.agent_approve)} className="rounded-xl bg-indigo-600 px-3 py-1.5 text-xs text-white disabled:opacity-60">Approve All</button><button type="button" onClick={() => void rejectAgentAction()} disabled={!selectedSession.pendingActions.length || Boolean(busy.agent_reject)} className="rounded-xl bg-rose-600 px-3 py-1.5 text-xs text-white disabled:opacity-60">Reject All</button><button type="button" onClick={() => void loadAgentSession(selectedSession.id)} className="rounded-xl border border-zinc-700 px-3 py-1.5 text-xs">Refresh Session</button></div><div className="space-y-2">{selectedSession.pendingActions.map((action) => <div key={action.id} className="rounded-xl border border-zinc-700 bg-zinc-900 p-2 text-sm"><div className="flex items-center justify-between"><div><p className="font-medium">{action.tool}</p><p className="text-xs text-zinc-400">{action.reason}</p></div><div className="flex gap-1"><button type="button" onClick={() => void approveAgentAction(action.id)} disabled={Boolean(busy.agent_approve)} className="rounded-lg bg-emerald-600 px-2 py-1 text-[11px] text-white disabled:opacity-60">Approve</button><button type="button" onClick={() => void rejectAgentAction(action.id)} disabled={Boolean(busy.agent_reject)} className="rounded-lg bg-rose-600 px-2 py-1 text-[11px] text-white disabled:opacity-60">Reject</button></div></div></div>)}{selectedSession.pendingActions.length === 0 && <p className="text-sm text-zinc-400">No pending actions.</p>}</div></div> : <p className="mt-2 text-sm text-zinc-400">Select a session to continue.</p>}</div>
                  <div className="surface-panel rounded-3xl p-4"><h3 className="text-base font-semibold">Transcript</h3>{selectedSession ? <div className="mt-2 max-h-[360px] space-y-2 overflow-auto">{selectedSession.transcript.map((item, idx) => <div key={`${item.timestampIso}-${idx}`} className="rounded-xl bg-zinc-900/70 p-2 text-sm"><div className="flex items-center justify-between"><span className="font-medium">{item.role}</span><span className="text-xs text-zinc-400">{fmt(item.timestampIso)}</span></div><p className="mt-1 whitespace-pre-wrap text-zinc-300">{item.content}</p></div>)}{selectedSession.transcript.length === 0 && <p className="text-sm text-zinc-400">No transcript yet.</p>}</div> : <p className="mt-2 text-sm text-zinc-400">No selected session.</p>}</div>
                </div>
              </section>
            )}
            ...
            {screen === "queue" && (
              <section className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  <div className="surface-panel rounded-3xl p-4"><p className="text-xs uppercase text-zinc-400">Queue Enabled</p><p className="mt-1 text-2xl font-semibold">{queueRuntime ? String(queueRuntime.enabled) : "-"}</p></div>
                  <div className="surface-panel rounded-3xl p-4"><p className="text-xs uppercase text-zinc-400">Queue Ready</p><p className="mt-1 text-2xl font-semibold">{queueRuntime ? String(queueRuntime.ready) : "-"}</p></div>
                  <div className="surface-panel rounded-3xl p-4"><p className="text-xs uppercase text-zinc-400">Group Queued</p><p className="mt-1 text-2xl font-semibold">{groupStatus ? groupStatus.queue.queued : "-"}</p></div>
                  <div className="surface-panel rounded-3xl p-4"><p className="text-xs uppercase text-zinc-400">Agent Pending</p><p className="mt-1 text-2xl font-semibold">{pendingActionCount}</p></div>
                </div>
                <div className="surface-panel rounded-3xl p-4"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><h2 className="text-base font-semibold">Queue Runtime + Group Queue</h2><div className="flex gap-2"><select value={groupQueueFilter} onChange={(e) => setGroupQueueFilter(e.target.value as "all" | GroupPostStatus)} className="rounded-xl border border-zinc-700 px-2 py-1.5 text-sm"><option value="all">all</option><option value="queued">queued</option><option value="processing">processing</option><option value="sent">sent</option><option value="failed">failed</option></select><button type="button" onClick={() => void Promise.all([loadQueueRuntime(), loadGroupStatus(), loadGroupQueue()])} className="rounded-xl border border-zinc-700 px-3 py-1.5 text-sm">Refresh</button></div></div><div className="grid gap-4 xl:grid-cols-2"><div className="rounded-xl bg-zinc-900/70 p-3 text-sm">{queueRuntime ? <div className="space-y-1"><p>queueName: <span className="font-semibold">{queueRuntime.queueName}</span></p><p>attempts: <span className="font-semibold">{queueRuntime.attempts}</span></p><p>concurrency: <span className="font-semibold">{queueRuntime.concurrency}</span></p><p>timeoutMs: <span className="font-semibold">{queueRuntime.timeoutMs}</span></p><p>reason: <span className="font-semibold">{queueRuntime.reason || "-"}</span></p></div> : <p className="text-zinc-400">No runtime data.</p>}</div><div className="rounded-xl bg-zinc-900/70 p-3 text-sm">{groupStatus ? <div className="space-y-1"><p>schedulerEnabled: <span className="font-semibold">{String(groupStatus.scheduler.enabled)}</span></p><p>running: <span className="font-semibold">{String(groupStatus.scheduler.running)}</span></p><p>intervalMs: <span className="font-semibold">{groupStatus.scheduler.intervalMs}</span></p><p>batchSize: <span className="font-semibold">{groupStatus.scheduler.batchSize}</span></p><p>nextDue: <span className="font-semibold">{fmt(groupStatus.queue.nextDueAtIso)}</span></p></div> : <p className="text-zinc-400">No group status.</p>}</div></div><div className="mt-3 overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="text-left text-xs uppercase text-zinc-400"><th className="pb-2 pr-3">Item</th><th className="pb-2 pr-3">Status</th><th className="pb-2 pr-3">Next</th><th className="pb-2 pr-3">Attempts</th><th className="pb-2 pr-3">Targets</th></tr></thead><tbody className="divide-y divide-zinc-800">{groupQueue.map((item) => <tr key={item.id}><td className="py-2 pr-3"><div className="font-medium">{item.id}</div><div className="line-clamp-2 text-xs text-zinc-400">{item.content}</div></td><td className="py-2 pr-3"><span className={`rounded-full px-2 py-0.5 text-xs ${groupStatusClass(item.status)}`}>{item.status}</span></td><td className="py-2 pr-3 text-zinc-400">{fmt(item.nextPostAtIso)}</td><td className="py-2 pr-3">{item.attempts}</td><td className="py-2 pr-3 text-zinc-400">{item.targets.length}</td></tr>)}{groupQueue.length === 0 && <tr><td colSpan={5} className="py-6 text-center text-zinc-400">No queue items.</td></tr>}</tbody></table></div></div>
              </section>
            )}

            {screen === "settings" && (
              <section className="grid gap-4 xl:grid-cols-[1fr_0.9fr]">
                <div className="surface-panel rounded-3xl p-4"><h2 className="text-base font-semibold">Settings</h2><div className="mt-2 space-y-2"><input value={draftConfig.baseUrl} onChange={(e) => setDraftConfig((p) => ({ ...p, baseUrl: e.target.value }))} placeholder="base URL" className="w-full rounded-xl border border-zinc-700 px-3 py-2 text-sm" /><input type="password" value={draftConfig.apiKey} onChange={(e) => setDraftConfig((p) => ({ ...p, apiKey: e.target.value }))} placeholder="AGENT_API_KEY" className="w-full rounded-xl border border-zinc-700 px-3 py-2 text-sm" /><select value={draftConfig.role} onChange={(e) => setDraftConfig((p) => ({ ...p, role: e.target.value as AgentRole }))} className="w-full rounded-xl border border-zinc-700 px-3 py-2 text-sm"><option value="realtor_admin">realtor_admin</option><option value="ops">ops</option></select><div className="flex gap-2"><button type="button" onClick={saveSettings} className="inline-flex items-center rounded-xl bg-emerald-600 px-3 py-2 text-sm text-white"><Settings2 className="mr-1 h-4 w-4" />Save</button><button type="button" onClick={() => void probeHealth()} className="inline-flex items-center rounded-xl border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm"><RefreshCcw className="mr-1 h-4 w-4" />Ping</button></div></div></div>
                <div className="surface-panel rounded-3xl p-4"><h3 className="text-base font-semibold">Guardrails</h3><div className="mt-2 space-y-2 text-sm"><div className="rounded-xl bg-emerald-500/20 p-3 text-emerald-200">Use valid `x-agent-api-key` and `x-agent-role` headers.</div><div className="rounded-xl bg-sky-500/20 p-3 text-sky-200">Run preflight and approval before run.</div><div className="rounded-xl bg-amber-500/20 p-3 text-amber-200">Marketing should keep consentMode=required.</div></div></div>
              </section>
            )}
          </main>
        </div>
